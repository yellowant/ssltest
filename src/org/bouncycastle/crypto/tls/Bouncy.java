	...
	private final class CipherProbingClient extends DefaultTlsClient {
		...
		public Hashtable getClientExtensions() throws IOException {
			...
			TlsExtensionsUtils
					.addServerNameExtension(
							clientExtensions,
							new ServerNameList(new Vector<>(Arrays
									.asList(new ServerName(NameType.host_name,
											host)))));
			TlsExtensionsUtils.addHeartbeatExtension(clientExtensions,
					new HeartbeatExtension(HeartbeatMode.peer_allowed_to_send));
		...
		public void notifyAlertReceived(short alertLevel, short alertDescription) {
			if (alertLevel == AlertLevel.fatal
					&& AlertDescription.handshake_failure == alertDescription) {
				failed = true;
			}
	...
	public String[] determineCiphers(TestOutput pw) throws IOException {
		...
		try {
			for (int n = 0; n < 80; n++) {
				...
				String cipherDesc = cipherNames.get(selection) + " (0x"
						+ Integer.toHexString(selection) + ") " + cipherInfo;
				if (pw != null) {
		...
		int choice = choose(new int[]{worst, best}, sr);
